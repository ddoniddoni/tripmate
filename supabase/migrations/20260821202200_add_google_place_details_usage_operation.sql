alter table public.google_maps_usage_daily
drop constraint google_maps_usage_daily_operation_check;

alter table public.google_maps_usage_daily
add constraint google_maps_usage_daily_operation_check
check (operation in ('places_text_search', 'places_details', 'maps_javascript_load', 'routes_compute'));

create or replace function public.reserve_google_maps_usage(
  p_operation text,
...
begin
  if p_operation not in ('places_text_search', 'places_details', 'maps_javascript_load', 'routes_compute') then
